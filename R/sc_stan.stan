functions {
  ...
  vector ar_process(vector innovations,
    ...
    if(integrate_process) {
      ...
      real scale_fac = sqrt(T + dot_product(pow(0.98, seq_T), T - seq_T));
...
data {
  ...
  int<lower=0, upper=1> include_spillover;
  ...
  // Prior scale hyperparameter for the overall standard deviation of data.
  real<lower=0> overall_sd_prior_scale;
  ...
  real<lower=0, upper=1> phi_latent_ub;
...
transformed data {
  ...
  // Total number of observed and possibly observed times.
  int T_tot = T_times + T_pos;
  //real scale_mis = sd(to_vector(Y_obs));
  ...
  int S_spillover = include_spillover ? T_times - earliest_treated + 1 : 0;
  int M_spillover = 0;
...
parameters {
  ...
  // Unobserved response values.
  // ---------------------------------------------------------------------------
  // matrix[T_pos, N_units] Y_pos;
  // vector<lower = 0>[T_pos * N_units] sd_pos_0;
  // vector[T_pos * N_units] mean_pos;
  // real<lower = 0> tau_pos;
  ...
  matrix[T_tot, K_latent] factors_0;
  
  // Lag-1 autocorrelation for latent factor process.
  vector<lower=0, upper=1>[K_latent] factors_autocor_0;
  // ---------------------------------------------------------------------------
  ...
  vector<lower=0, upper=1>[L_covars] time_coefs_autocor_0;
  ...
  // Scale-normalized per-unit standard deviation of untreated outcomes.
  vector<lower=0>[N_units] overall_sd_0;
  
  // Fraction of variance explained by latent factor process.
  real<lower=0, upper=1> frac_var_latent_0;
...
transformed parameters {
  ...
  // Total observed and possibly observed response values.
  // matrix[T_tot, N_units] Y_tot = append_row(Y_pos, Y_obs);
  // vector[T_pos * N_units] sd_pos = tau_pos * sd_pos_0;
  ...
  factors_autocor = phi_latent_lb +
    (phi_latent_ub - phi_latent_lb) * factors_autocor_0;

  matrix[T_tot, K_latent] factors;
  for(k in 1:K_latent) {
    factors[:, k] = ar_process(factors_0[:, k], factors_autocor[k], 1, integrated_factors);
  }
  
  // Per-unit standard deviation of untreated outcomes.                             
  vector<lower=0>[N_units] overall_sd = overall_sd_prior_scale * overall_sd_0;
  
  // Per-unit latent mean process.
  real<lower=lambda_lb, upper=lambda_ub> frac_var_latent;
  frac_var_latent = lambda_lb + (lambda_ub - lambda_lb) * frac_var_latent_0;

  matrix[T_times, N_units] latent_means = factors[(T_pos+1):T_tot,:] 
                                          * factor_loadings_t;
  
  matrix[T_times, N_units] latent_component;
  for(n in 1:N_units) {
    latent_component[:, n] = (overall_sd[n] * sqrt(frac_var_latent) * latent_means[:,n]);
  }
  // ---------------------------------------------------------------------------
  ...
  time_coefs_autocor = pz_bounds[1] +
    (pz_bounds[2] - pz_bounds[1]) * time_coefs_autocor_0;

  matrix[T_tot, L_covars] time_coefs;
  ...
  if(L_covars > 0) {
    for(n in 1:N_units) {
      if(include_unit_coefs) {
        ...
        // TEST
        regression_component[,n] = overall_sd[n] * rows_dot_product(X[n], time_coefs);
      }
    } 
  } else {
    regression_component = rep_matrix(0, T_times, N_units);
  }
...
model {
  // Possibly observed responses.
  // to_vector(Y_pos) ~ normal(mean_pos, sd_pos);
  // mean_pos ~ normal(0, scale_mis);
  // sd_pos_0 ~ normal(0, 1);
  // tau_pos ~ normal(0, scale_mis);
  ...
  for(n in 1:N_units) {
    ...
    int n_treated_start = min({num_treated_before[n]+1, total_treated});
    ...
    real sd_n = sqrt(1 - frac_var_latent) * overall_sd[n];
    
    // Error likelihood for unit n.
    // Y_obs[:,n] ~ student_t(5, mean_n, sd_n);
    Y_obs[:,n] ~ normal(mean_n, sd_n);
  }
  // ---------------------------------------------------------------------------
  
  // (Roughly) unit priors for scale-normalized quantities
  // ---------------------------------------------------------------------------
  to_vector(factors_0) ~ normal(0, 1);
  // to_vector(factors_0) ~ student_t(4, 0, 1);
  ...
  // time_coefs_mean ~ normal(0, 1);
  
  unit_intercept_0 ~ normal(0, 1);
  
  overall_sd_0 ~ inv_gamma(10, 9);
  ...
  random_effects_flat_upper ~ inv_gamma(8, 9);
  ...
  //   
  //   for(k in 2:K_latent){
  //     factor_loadings_upper[k,k] ~ inv_gamma(8, 9);
  //   }
  //   
  // }
  // --------------------------------------------------------------------------
...
generated quantities {
  ...
  // similarity = to_row_vector(factor_loadings_t[, 2:N_units]'
  //                              * factor_loadings_t[, 1])
  //                ./ sqrt(columns_dot_self(factor_loadings_t[,2:N_units])
  //                          * dot_self(factor_loadings_t[,1]));
