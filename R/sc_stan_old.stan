...
transformed parameters {
  ...
  for(k in 1:K_latent) {
    /*factors[1, k] = sqrt(1 - (factors_autocor[k]^2)) * factors_0[1, k];
    for(t in 2:T_times) {
      factors[t, k] = (factors_autocor[k] * factors[t-1, k])
                        + (sqrt(1 - (factors_autocor[k]^2)) * factors_0[t, k]);
    }*/
  ...
  for(l in 1:L_covars) {
    // time_coefs[1, l] = time_coefs_sd[l] * sqrt(1 - (time_coefs_autocor[l]^2))
    //                                     * time_coefs_0[1, l];
    //                     
    // for(t in 2:T_times) {
    //   time_coefs[t, l] =
    //     (time_coefs_autocor[l] * time_coefs[t-1, l])
    //       + (time_coefs_sd[l] * sqrt(1 - (time_coefs_autocor[l]^2)) 
    //                           * time_coefs_0[t, l]);
    // }
...
model {
  ...
  for(n in 1:N_units) {
    ...
    // vector[T_times] iota_n = rep_vector(0, T_times);
    // iota_n[earliest_treated:T_times] = spillover_effects[n,:];
...
generated quantities {
  ...
  for(n in 1:N_units) {
    latent_trends[:,n] = unit_intercept[n]
                           + regression_component[,n]
                           + (overall_sd[n] * sqrt(frac_var_latent) 
                                            * latent_means[:,n]);
  }
